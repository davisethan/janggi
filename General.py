# Author: Ethan Davis
# Date: 2/27/21
# Description: Janggi Korean chess general piece
...
class General(Piece):
    """
    General (Piece) handles finding general move destinations from source filtering moves with board layout
    """

    def all_destinations_from_source(self, source):
        """
        General all destinations from source
...
class BlueGeneral(General):
    """
    BlueGeneral (Piece) handles finding general move destinations from source filtering moves with board layout
    """

    def destinations_from_source(self, source, board, deep_check=True):
        """
        Blue general destinations from source
        :param source: [Tuple[int]] Board source coordinate
        :param board: [Board]
...
class RedGeneral(General):
    """
    RedGeneral (Piece) handles finding general move destinations from source filtering moves with board layout
    """

    def destinations_from_source(self, source, board, deep_check=True):
        """
        Red general destinations from source
        :param source: [Tuple[int]] Board source coordinate
        :param board: [Board]
        :param deep_check: [bool] Piece which checks other general must protect own general
        :return: [Dict[Tuple[int], Tuple[Tuple[int]]]] Destination goes to source to destination path
